...
It is intended for repeated monthly use with a new workbook upload each month.
...
1. Upload this month's workbook.
2. Review the parsed data.
3. Check validation messages.
4. Enter any required manual values.
5. Run the calculation.
6. Download the finished Excel output.

The app is built around a reusable monthly template:

- workbook values can change from month to month
- row counts can vary within the configured block areas
- reference values, targets, percentages, and totals are not hardcoded for one month
- structure is validated before calculation is allowed
- if the workbook differs too much from the expected layout, the app stops calculation and shows a clear message
