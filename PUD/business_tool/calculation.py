...
def _blocked_calculation_result(message: str) -> CalculationResult:
    return CalculationResult(
        block_results=[],
        warnings=[CalculationWarning(block_name="Workbook", message=message)],
        block_summary_frame=pd.DataFrame(),
        summary=CalculationSummary(
            rows_prepared=0,
            ready_rows=0,
...
def run_calculation_engine(
    ...
    if validation_result.error_count > 0:
        return _blocked_calculation_result(
            "Calculation was not run because the uploaded workbook needs review before results can be prepared."
        )
