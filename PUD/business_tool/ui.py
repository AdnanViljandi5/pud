...
def _render_header() -> None:
    ...
    st.caption("Upload the monthly workbook, review the information, complete any required values, and prepare the results.")
...
def _render_upload_section() -> None:
    ...
    st.caption("Upload this month's Excel workbook. The app will check the structure before calculation.")
...
def _render_calculation_section(parsed_workbook: ParsedWorkbook | None, validation_result: ValidationResult | None) -> None:
    ...
    if validation_result.error_count > 0:
        st.error(
            "Calculation is not available yet because the uploaded workbook does not match the expected structure closely enough. "
            "Please review the file and the data checks before continuing."
        )
        return

    if validation_result.warning_count > 0:
        st.warning("Some values still need review. You can run the calculation after confirming the items listed in Check Data.")
